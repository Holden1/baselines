...
def parse_args():
    ...
    parser.add_argument("--num-steps", type=int, default=int(2e7), help="total number of steps to run the environment for")
...
def maybe_train():
    if (num_iters > max(5 * args.batch_size, args.replay_buffer_size // 20) and
                ...
                return True
    return False
def maybe_train_for_seconds(num_seconds):
    t1 = datetime.now()
    while 1:
        if not maybe_train():
            ...
            print("Training for",num_seconds-(datetime.now()-t1).seconds,"seconds")

        if (datetime.now()-t1).seconds > num_seconds:
            break
def train_while_resetting():
...
if __name__ == '__main__':
    ...
    with U.make_session(4) as sess:
        ...
        obs,rew,done = gymenv.frame_step(6)
        #print("Done------------------- : ",done)
        if done:
            gymenv.reset()
            obs, rew, done = gymenv.frame_step(6)
        ...
        while True:
            ...
            reset = False
            new_obs, rew, done = gymenv.frame_step(action)
            reward_since_reset+=rew
            replay_buffer.add(copy.copy(obs), action, rew, copy.copy(new_obs), float(done))
            obs = new_obs
            if done:
                ...
                while not gymenv.can_reset():
                    maybe_train()
                    if (time.time() - start_train)>30:
                        #Assume loading screen is stuck
                        print("loading screen stuck, killing processes")
                        gymenv.kill_processes()

                #Train for another 5 seconds to make sure loading screen is over
                maybe_train_for_seconds(10)
                train_while_resetting()
                
                obs,_,_ = gymenv.frame_step(6)
                reset = True
            ...
            if done:
                ...
                q_vals = debug["q_values"](np.asarray(obs).reshape(1, 4, 84, 84))
