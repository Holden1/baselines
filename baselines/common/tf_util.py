...
def sum(x, axis=None, keepdims=False):
    ...
    return tf.compat.v1.reduce_sum(x, axis=axis, keep_dims=keepdims)

def mean(x, axis=None, keepdims=False):
    axis = None if axis is None else [axis]
    return tf.compat.v1.reduce_mean(x, axis=axis, keep_dims=keepdims)
...
def max(x, axis=None, keepdims=False):
    axis = None if axis is None else [axis]
    return tf.compat.v1.reduce_max(x, axis=axis, keep_dims=keepdims)

def min(x, axis=None, keepdims=False):
    axis = None if axis is None else [axis]
    return tf.compat.v1.reduce_min(x, axis=axis, keep_dims=keepdims)
...
def wndense(x, size, name, init_scale=1.0):
    ...
    g = tf.compat.v1.get_variable(name + "/g", [size], initializer=tf.constant_initializer(init_scale))
    b = tf.compat.v1.get_variable(name + "/b", [size], initializer=tf.constant_initializer(0.0))
...
class Module(object):
    ...
    def trainable_variables(self):
        ...
        return tf.get_collection(tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, self.scope)

    @property
    def variables(self):
        assert self.scope is not None, "need to call module once before getting variables"
        return tf.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, self.scope)
